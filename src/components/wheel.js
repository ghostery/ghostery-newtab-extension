...
export default {
  ...
  current: '',
  tooltip: ({ render }) => render().querySelector('gh-tooltip'),
  render: ({ categories, data, current }) => html`
    <template layout="grid relative">
      ...
      >
        <span slot="content">
          ${trackers.labels[current]}: ${categories.reduce(count(current), 0)}
        </span>
      </gh-tooltip>
          ...
          ([key, { value, offset }]) =>
            svg`
              <path
                ...
                stroke="${trackers.backgroundColors[key]}"
